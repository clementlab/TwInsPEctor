...
def main():
    ...
    if args.recoding_mode:
        ...
        build_compound_reference_alignments(args.wt_seq, args.twin_seq, args.peg_spacers[0], args.peg_spacers[1], spacer_info, twinpe_8cat_results_folder, recoding_mode=args.recoding_mode)
        
        crispresso_cmd = build_crispresso_command(args=args, wt_seq=args.wt_seq, twin_seq=args.twin_seq, crispresso_output_folder=crispresso_run_folder, spacer_info=spacer_info, twinpe_8cat_output_folder=twinpe_8cat_results_folder)
        ...
        comp_ref_seq_a, wt_aln_seq_a, twin_aln_seq_a, comp_ref_seq_b, wt_aln_seq_b, twin_aln_seq_b = build_compound_reference_alignments(args.wt_seq, args.twin_seq, args.peg_spacers[0], args.peg_spacers[1], spacer_info, twinpe_8cat_results_folder, recoding_mode=args.recoding_mode)
        # compound_ref_seq = build_compound_reference(args.wt_seq, args.twin_seq)

        crispresso_cmd_a = build_crispresso_command(args=args, comp_ref_seq=comp_ref_seq_a, crispresso_output_folder=crispresso_run_a_folder, spacer_info=spacer_info, twinpe_8cat_output_folder=twinpe_8cat_results_folder, run_label="a")
        crispresso_cmd_b = build_crispresso_command(args=args, comp_ref_seq=comp_ref_seq_b, crispresso_output_folder=crispresso_run_b_folder, spacer_info=spacer_info, twinpe_8cat_output_folder=twinpe_8cat_results_folder, run_label="b")
...
def parse_args():
    ...
    parser.add_argument("-rm", "--recoding_mode", action="store_true", help="Run in recoding mode when there are only base substitutions.")
...
def build_crispresso_command(args=None, comp_ref_seq=None, wt_seq=None, twin_seq=None, crispresso_output_folder=None, spacer_info=None, twinpe_8cat_output_folder=None, run_label=None):
...
def analyze_visualize_sample(
    ...
    if args.recoding_mode:
        ...
        df_alleles_a_results = categorize_alleles(crispresso_output_folder=crispresso_output_folder_a, comp_ref_seq=comp_ref_seq_a, wt_aln_seq=wt_aln_seq_a, twin_aln_seq=twin_aln_seq_a, num_changes_to_check=args.num_changes_to_check, ignore_extraspacer_deletions=args.ignore_extraspacer_deletions)

        df_alleles_b_results = categorize_alleles(crispresso_output_folder=crispresso_output_folder_b, comp_ref_seq=comp_ref_seq_b, wt_aln_seq=wt_aln_seq_b, twin_aln_seq=twin_aln_seq_b, num_changes_to_check=args.num_changes_to_check, ignore_extraspacer_deletions=args.ignore_extraspacer_deletions)

        df_alleles_final_a, df_alleles_final_b = collapse_allele_categories(df_alleles_a_results["df_alleles"], df_alleles_b_results["df_alleles"])
        ...
        results_final = analyze_collapsed_categorized_alleles(
            ...
            del_region_len_a=df_alleles_a_results["del_region_len"], 
            ...
            del_region_len_b=df_alleles_b_results["del_region_len"], 
    ...
    plot_per_base_pos_barplots(
        ...
        # ins_start_a=df_alleles_a_results["ins_start"] if not args.recoding_mode else None, 
        # ins_end_a=df_alleles_a_results["ins_end"] if not args.recoding_mode else None, 
        # del_start_a=df_alleles_a_results["del_start"] if not args.recoding_mode else None,  
        # del_end_a=df_alleles_a_results["del_end"] if not args.recoding_mode else None,  
...
def get_ref_base_changes(comp_ref_seq=None, wt_aln_seq=None, twin_aln_seq=None, wt_seq=None, twin_seq=None):
    ...
    for idx in range(len(comp_ref_seq)):
        ...
        if wt_base != '-' and twin_base == '-':
            ...
            raise ValueError('Subsitution detected in Repalcement mode.')
...
def get_recoding_read_match_array(bp_changes_arr, read_map):
    match_arr = []
    for (idx, wt_base, twin_base) in bp_changes_arr:
...
def categorize_alleles(crispresso_output_folder=None, comp_ref_seq=None,  wt_aln_seq=None, twin_aln_seq=None, num_changes_to_check=2, ignore_extraspacer_deletions=False):
    ...
    for idx, allele in df_alleles.iterrows():
        ...
        # if len(ins_match_arr) >= num_changes_to_check:  # likely not important to check this for replacement mode
...
def analyze_collapsed_categorized_alleles(
    ...
    for sequence_key, allele in df_alleles_final_a.iterrows():
        ...
        for pos_idx, match in zip(range(len(bp_changes_arr_a)), del_match_arr+ins_match_arr): # match_arr):
            ...
            if match == 'D':
                full_deletion_counts_a[pos_idx] += allele['#Reads']
            if match == "I":
        ...
        for pos_idx, match in zip(range(len(bp_changes_arr_b)), ins_match_arr+del_match_arr): # match_arr):
            if match == 'T':
                full_edit_counts_b[pos_idx] += allele['#Reads']
                # if has_indel:
                #     full_edit_counts_with_indels[pos_idx] += allele['#Reads']
                # else:
                #     full_edit_counts_no_indels[pos_idx] += allele['#Reads']
            if match == 'D':
                full_deletion_counts_b[pos_idx] += allele['#Reads']
            if match == "I":
                full_insertion_counts_b[pos_idx] += allele["#Reads"]
            if match == "S":
                full_substitution_counts_b[pos_idx] += allele["#Reads"]
    ...
    #     os.path.join(twinpe_8cat_results_folder, "c8.a.detailed_allele_table_with_categories.csv"), 
    #                  index=False
    # )
    # df_alleles_final_b.to_csv(
    #     os.path.join(twinpe_8cat_results_folder, "c8.b.detailed_allele_table_with_categories.csv"), 
    #                  index=False
    # )
...
def plot_edit_read_counts_with_indels(
    ...
    ax1.bar(indices, edit_counts,
            label="Total TPEs",
            color=category_colors["Perfect PE"])
...
def plot_categorical_allele_tables(
    ...
    for cat, name_a, name_b, name_r in [
        ("Perfect_PE", "b1.a.Perfect_PE", "b1.b.Perfect_PE", "b1.Perfect_PE"),
        ("PE_Indel", "b2.a.PE_Indel", "b2.b.PE_Indel", "b2.PE_Indel"),
        ("Left_Flap", "b3.a.Left_Flap", "b3.b.Left_Flap", "b3.Left_Flap"),
        ("Right_Flap", "b4.a.Right_Flap", "b4.b.Right_Flap", "b4.Right_Flap"),
        ("Imperfect_PE", "b5.a.Imperfect_PE", "b5.b.Imperfect_PE", "b5.Imperfect_PE"),
...
#     Files that contain `.a.` or `.b.` (e.g., `b1.a.Perfect_PE.png`)
#     are rendered inside the same card with toggle buttons so viewers
#     can switch between Run A / Run B.

#     Images are displayed fully responsive (entire figure visible) by default,
#     and individual figures can be clicked to toggle multi-level zoom in a
#     scrollable view.
#     """
...
#             groups.setdefault(key, {"title": key, "variants": {}})
#             groups[key]["variants"][label] = img
#         else:
...
#         if "single" in entry:
#             img_rel = html.escape(entry["single"])
...
#             for label in sorted(entry["variants"]):
#                 img_rel = html.escape(entry["variants"][label])

#                 panel_id = f"panel-{uid}-{label}"
#                 active = "active" if label == "a" else ""
#                 hidden = "" if label == "a" else "d-none"
#                 label_text = "A" if label == "a" else "B"
...
#     /* Default: no inner scroll box, let the page scroll */
#     .img-panel {{
...
#     /* When zoomed, constrain height and enable scroll inside the panel */
...
#     /* Any zoom level > 1 uses this cursor */
#     .report-img.zoomed {{
#       cursor: zoom-out;
#     }}
#   </style>
...
#     // Multi-level zoom per image: cycle through scale 1 -> 1.7 -> 3 -> 1 ...
...
#           }}
#         }} else {{
#           // Zoomed-in view with scrollable panel
...
#     out_path = os.path.join(twinpe_8cat_results_folder, html_filename)
...
def build_combined_html_report(twinpe_8cat_results_folder, twinpe_8cat_results_folder_less, html_filename="TwinPE_report.html"):
    ...
    ab_titles = {
        ...
        "b5.Imperfect_PE.png": "Imperfect TPE Alleles",
    ...
    for key in sorted(groups):
        ...
        if "single" in entry:
            ...
            img_rel = html.escape(entry["single"])
            img_rel = os.path.join("Outputs", img_rel)
            cards_html.append(f"""
            <div class="card mb-4 shadow-sm">
              <div class="card-header fw-semibold">{title}</div>
              <div class="card-body text-center">
            ...
            for idx, (v_key, v_info) in enumerate(variant_items):
                # point HTML to Outputs/<filename>
                img_rel = html.escape(v_info["img"])
                img_rel = os.path.join("Outputs", img_rel)
